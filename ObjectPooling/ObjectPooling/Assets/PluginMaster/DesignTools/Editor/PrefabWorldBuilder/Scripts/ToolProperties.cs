...
namespace PluginMaster
{
    public class ToolProperties : EditorWindow
    {
        ...
        {
            if (PWBCore.staticData.undoToolProperties) Undo.RegisterCompleteObjectUndo(this, UNDO_MSG);
            GUI.FocusControl(null);
        ...
        private void LineBaseGUI(LineSettings lineSettings)
        {
            using (var check = new EditorGUI.ChangeCheckScope())
            {
                ...
                using (new GUILayout.VerticalScope(EditorStyles.helpBox))
                {
                    ...
                    if (settings.mode != PaintOnSurfaceToolSettings.PaintMode.ON_SHAPE)
                        settings.parallelToTheSurface = EditorGUILayout.ToggleLeft("Place objects parallel to the surface",
                            settings.parallelToTheSurface);
                ...
                using (new GUILayout.VerticalScope(EditorStyles.helpBox))
                {
                    ...
                    if (!extrudeSettings.sameParentAsSource)
                    {
                        ...
                        if (extrudeSettings.autoCreateParent) extrudeSettings.createSubparentPerPrefab
                                = EditorGUILayout.ToggleLeft("Create sub-parent per prefab", extrudeSettings.createSubparentPerPrefab);
                    ...
                    using (new GUILayout.VerticalScope(EditorStyles.helpBox))
                    {
                        ...
                        if (!mirrorSettings.sameParentAsSource)
                        {
                            ...
                            if (mirrorSettings.autoCreateParent)
                                mirrorSettings.createSubparentPerPrefab = EditorGUILayout.ToggleLeft("Create sub-parent per prefab",
