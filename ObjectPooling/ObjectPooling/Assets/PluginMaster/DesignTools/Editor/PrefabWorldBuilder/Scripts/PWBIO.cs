...
namespace PluginMaster
{
    ...
    public static partial class PWBIO
    {
        ...
        private static void AskIfWantToSave(System.Action Save)
        {
        ...
        private static void AskIfWantToSave(int toolState, int editState, System.Action Save)
        {
            switch (PWBCore.staticData.unsavedChangesAction)
            {
                case PWBData.UnsavedChangesAction.ASK:
                    if (toolState == editState) AskIfWantToSave(Save);
                    break;
                case PWBData.UnsavedChangesAction.SAVE:
                    if (toolState == editState) Save();
        ...
        public static void DuringSceneGUI(SceneView sceneView)
        {
            ...
            {
                if (tool == ToolManager.PaintTool.LINE && _lineData != null && _lineData.state != LineData.LineState.NONE)
                    ResetLineState();
                else if (tool == ToolManager.PaintTool.SHAPE
                    && _shapeData != null && _shapeData.state != ShapeData.ShapeState.NONE)
                    ResetShapeState();
                else if (tool == ToolManager.PaintTool.TILING
                    && _tilingData != null && _tilingData.state != TilingData.TilingState.NONE)
                    ResetTilingState();
        ...
        private static GameObject[] Paint(IPaintToolSettings settings,
            string commandName = PAINT_CMD, bool addTempCollider = true, string toolObjectId = null)
        {
            ...
            foreach (var item in _paintStroke)
            {
                ...
                {
                    item.parent = GetParent(settings, item.prefab.name, true, toolObjectId);
                }
        ...
        private static Transform GetParent(IPaintToolSettings settings, string prefabName,
            bool create, string toolObjectId = null)
        {
            ...
            if (settings.autoCreateParent && create)
            {
                ...
                string subParentKey = (settings.createSubparentPerPalette
                    ? PaletteManager.selectedPalette.name : NO_PALETTE_NAME) + PARENT_KEY_SEPARATOR
                    + (settings.createSubparentPerTool ? ToolManager.tool.ToString() : NO_TOOL_NAME) + PARENT_KEY_SEPARATOR
                    + (string.IsNullOrEmpty(toolObjectId) ? NO_OBJ_ID : toolObjectId) + PARENT_KEY_SEPARATOR
                ...
                if (settings.createSubparentPerPalette || settings.createSubparentPerTool
                    || settings.createSubparentPerBrush || settings.createSubparentPerPrefab)
                {
                    create = !(_subParents.ContainsKey(subParentKey));
                    if (!create && _subParents[subParentKey] == null) create = true;
                    if (create)
                    {
                        string GetSubParentKey(string palette = NO_PALETTE_NAME, string tool = NO_TOOL_NAME,
                            string id = NO_OBJ_ID, string brush = NO_BRUSH_NAME, string prefab = NO_PREFAB_NAME)
                            => palette + PARENT_KEY_SEPARATOR + tool + PARENT_KEY_SEPARATOR + id + PARENT_KEY_SEPARATOR
                                + brush + PARENT_KEY_SEPARATOR + prefab;
                        Transform CreateSubParent(string key, string name, Transform transformParent)
                        {
                            var subParentTransform = transformParent.Find(name);
                        ...
                        {
                            var key = GetSubParentKey(palette, tool, id, brush, prefab);
                        ...
                        var keyPlaletteName = keySplitted[0];
                        var keyToolName = keySplitted[1];
                        var keyToolObjId = keySplitted[2];
                        var keyBrushName = keySplitted[3];
                        var keyPrefabName = keySplitted[4];
                        ...
                        if (keyBrushName != NO_BRUSH_NAME)
                            CreateSubParentIfDoesntExist(keyBrushName, keyPlaletteName,
                                keyToolName, keyToolObjId, keyBrushName);
