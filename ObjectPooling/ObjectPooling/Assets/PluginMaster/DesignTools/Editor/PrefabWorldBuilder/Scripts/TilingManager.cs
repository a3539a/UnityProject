...
namespace PluginMaster
{
    ...
    public class TilingData : ISerializationCallbackReceiver
    {
        ...
        public static string HexId(long value) => "Tiling_" + value.ToString("X");
        public static string nextHexId => HexId(DateTime.Now.Ticks);
    ...
    public static partial class PWBIO
    {
            ...
            if (askIfWantToSave)
            {
                void Save()
                {
                    if (SceneView.lastActiveSceneView != null)
                        TilingStrokePreview(SceneView.lastActiveSceneView.camera);
                    CreateTiling();
                }
                AskIfWantToSave((int)_tilingData.state, (int)TilingData.TilingState.EDIT, Save);
            }
        ...
        {
            Paint(TilingManager.settings, PAINT_CMD, true, TilingData.nextHexId);
        }
        ...
        private static void TilingStateEdit(Camera camera)
        {
                ...
                && Event.current.type == EventType.MouseDrag && Event.current.shift)
            {
               ...
               && Event.current.keyCode == KeyCode.LeftArrow) RotateTiling(90, Vector3.down, true);
                ...
                && Event.current.keyCode == KeyCode.UpArrow) RotateTiling(90, Vector3.forward, true);
            else if (Event.current.type == EventType.KeyDown && Event.current.control && Event.current.shift
                && Event.current.keyCode == KeyCode.DownArrow) RotateTiling(90, Vector3.back, true);
